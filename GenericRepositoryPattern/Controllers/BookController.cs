...
namespace GenericRepositoryPattern.Controllers
{
    ...
    {
        private IGenericRepository<BookModel> _bookrepository;
        
        public BookController(IGenericRepository<BookModel> bookrepository)
        {
        ...
        public async Task<IActionResult> addNewBook(BookModel objBookModel)
        {
            ...
            var result = await _bookrepository.addNew(objBookModel);
            if (result == -1)
        ...
        {
            var result = await _bookrepository.deleteNew(id);
            if (result == -1)
                return NoContent();
        ...
        public async Task<IActionResult> updateNewBook([FromRoute]int id, [FromBody]JsonPatchDocument bookModel)
        {
            ...
            var result = await _bookrepository.updateNew(book);
            if (result == -1)
                return StatusCode(500, "Something went wrong while processing the request");
